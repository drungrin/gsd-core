...
describe('plan-review-convergence: --cursor/--qwen reviewer whitelist (#2272)', () => {
  ...
  test('[behavioral] the deployed extraction passes --cursor/--qwen through (not the --codex fallback)', (t) => {
    ...
    // Regressions: default + existing flags unchanged; an unknown reviewer still
    // falls back to --codex as before.
    assert.strictEqual(run('5'), '--codex', 'no reviewer flag → --codex default preserved');
    assert.strictEqual(run('5 --bogus-reviewer'), '--codex', 'unknown reviewer flag → --codex fallback preserved');
