...
**Phase issues:** `sync` mirrors every roadmap phase into a repository Issue labeled `gsd:phase`, carrying a stable `<!-- gsd:phase id="NN" -->` identity marker and a delimited `<!-- gsd:begin -->`/`<!-- gsd:end -->` region holding the phase's goal, success criteria, and requirement IDs. Text a developer writes outside that region survives every re-sync byte-for-byte; an edit made inside it is regenerated from `ROADMAP.md` the next time the phase's content changes, and GSD cannot detect that it happened. A damaged region self-heals (no fence pair) or is reported for manual repair (an unbalanced or duplicated pair) rather than guessed at. Issues are never closed, reopened, deleted, commented on, or relabeled; a phase removed from the roadmap has its issue reported as an orphan and left alone.

**Plan sub-issues and task rendering:** every `<NN>-<PP>-PLAN.md` mirrors to a native GitHub sub-issue of its phase issue (`addSubIssue`, GitHub's own hierarchy — not a body-text link), carrying its own `<!-- gsd:plan id="NN-PP" -->` marker and the same fenced-region contract phase issues use. A plan with a sibling SUMMARY.md is closed with Status `Done`; the plan at `STATE.md`'s current position is open with Status `In Progress`; every other plan is open with Status `Todo` — `Status` is GSD-owned, so a hand-set `Blocked`/`Deferred` reverts on the next sync, and a plan issue reopens if its SUMMARY.md disappears. The plan's `Wave`, `Autonomous`, `GSD ID`, `Phase`, and `Requirements` custom fields populate from PLAN.md frontmatter, and dependencies render as native cross-issue references under a `## Depends On` line. Tasks render inside the same region as a non-interactive, glyph-prefixed list (`✓`/`▶`/`○`) rolled up from the plan's own status — never a clickable checkbox. A phase approaching GitHub's 100-sub-issues-per-parent ceiling gets a warning at 90 from both `status` and `sync`, which never blocks a dispatch; an orphaned plan sub-issue (deleted or renumbered PLAN.md) is reported by name and never closed, unlinked, commented on, or deleted.

**Views bootstrap:** `init` also creates and repairs five GSD-owned views (Roadmap, Board, Table-by-Phase, By-Wave, Backlog) with their layout, filter, and (where declared) visible fields, and retypes the layout of whichever view sits leftmost on the board — the tab it opens on by default — governed by the configurable `github_sync.view.layout` key. GSD owns `name`/`layout`/`filter` on its five views and converges them back on every run; grouping is the one view property that survives a developer's own edit, because the GraphQL API has no write path for it. What the API cannot reach — grouping, milestone swimlanes, Insights charts, and choosing which view is leftmost — is left as a documented manual remainder for a developer to finish once in the GitHub UI; see the reference documentation for the full, reasoned ceiling.

**Configuration:** `github_sync.enabled` (default `false`), `github_sync.target.*`, `github_sync.project_title`, `github_sync.view.layout` (default `board`).
